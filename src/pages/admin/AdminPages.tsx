...
const assets = [
  { id: "TS-1021", name: "Rolex Submariner Date", category: "Đồng hồ", status: "PUBLISHED", image: "/assets/featured-rolex-v2.png" },
  { id: "TS-1022", name: "Nhẫn kim cương 3.01ct", category: "Trang sức", status: "APPROVED", image: "/assets/diamond-gia-v2.png" },
  { id: "TS-1023", name: "Mercedes-Benz S450L", category: "Xe cộ", status: "UNDER_REVIEW", image: "/assets/catalog-mercedes-s450.png" },
  { id: "TS-1024", name: "Hermès Birkin 30", category: "Thời trang", status: "DRAFT", image: "/assets/catalog-hermes-birkin.png" },
];
...
export function AdminAssetsPage() {
  ...
  return (
    <AdminPage
      ...
      intro="Theo dõi hình ảnh, hồ sơ và trạng thái xử lý của từng tài sản đấu giá."
    >
      <Filters />
      <Table headers={["Mã", "Hình ảnh", "Tài sản", "Trạng thái", "Hồ sơ", "Thao tác"]}>
        {assetRows.map((x, i) => (
          <tr key={x.id}>
            <td>{x.id}</td>
            <td>
              <ResilientImage
                className="admin-asset-thumbnail"
                src={x.image}
                alt={x.name}
              />
            </td>
            <td>
              <div className="admin-asset-identity">
                <strong>{x.name}</strong>
                <small>{x.category}</small>
              </div>
            </td>
            <td><Badge tone={i < 2 ? "success" : "warning"}>{assetStatusLabel[x.status] ?? x.status}</Badge></td>
            <td>{x.files}</td>
            <td>
