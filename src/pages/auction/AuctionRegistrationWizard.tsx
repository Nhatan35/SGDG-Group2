import { AlertCircle, CheckCircle2, ChevronLeft, FileText, Gavel, ShieldCheck, X } from 'lucide-react'
...
import '../../styles/auction-registration-deposit.css'
...
const steps = [
  ...
  ['review', 'Kiểm tra và xác nhận'],
  ['deposit', 'Đặt cọc và gửi'],
] as const
...
function RegistrationSummaryStep({ auction, acknowledged, onAcknowledge }: { auction: Auction; acknowledged: boolean; onAcknowledge: (value: boolean) => void }) {
  return <>
    <p className="registration-eyebrow">BƯỚC 1 / 5</p><h2>Kiểm tra phiên bạn muốn đăng ký</h2>
    ...
    <section className="registration-section"><h3>Điều kiện chính</h3><div className="registration-trust-grid">
      {['Thành viên và KYC sẽ được kiểm tra bằng tham chiếu mock.', 'Tiền cọc được xác nhận ở bước cuối cùng.', 'Quy tắc phiên là bản snapshot chỉ đọc.', 'Gửi đăng ký chưa đồng nghĩa được phê duyệt.'].map((copy) => <div className="registration-trust-item" key={copy}><ShieldCheck/><p>{copy}</p></div>)}
    </div></section>
...
function RegistrationEligibilityStep({ scenario }: { scenario: Scenario }) {
  ...
  const items = [
    ...
    ['Ví thanh toán', 'Sẵn sàng đặt cọc', 'Financial Mock xác nhận ví có thể được sử dụng tại bước đặt cọc cuối cùng.', 'Financial Management Mock', 'WAL-•••-6830'],
  ...
  const state = scenario === 'manual-review' ? ['Cần xem xét thêm', 'Chưa có quyết định cuối cùng. Bạn không cần gửi lại thông tin.'] : scenario === 'blocked-kyc' ? ['Chưa đáp ứng điều kiện', 'Tham chiếu KYC hiện chưa đáp ứng điều kiện của phiên đấu giá.'] : ['Chưa có tham chiếu', 'Financial Mock chưa cung cấp trạng thái ví để tiếp tục đặt cọc.']
  return <>
    <p className="registration-eyebrow">BƯỚC 2 / 5</p><h2>Kiểm tra điều kiện tham gia</h2>
...
function RegistrationRulesStep({ auction, declarations, onChange, onOpenRules }: { auction: Auction; declarations: boolean[]; onChange: (index: number) => void; onOpenRules: () => void }) {
  ...
  return <>
    <p className="registration-eyebrow">BƯỚC 3 / 5</p><h2>Xác nhận quy tắc phiên</h2><p className="registration-support">Đọc bản snapshot chỉ đọc và xác nhận các nội dung trước khi kiểm tra lần cuối.</p>
...
function RegistrationReviewStep({ auction, final, onFinal, go }: { auction: Auction; final: boolean; onFinal: (value: boolean) => void; go: (index: number) => void }) {
  const sections = [['Phiên đấu giá', `${auction.assetName} · ${auction.code} · Trang sức · ${formatMoney(auction.startPrice)} · Hạn đăng ký: ${deadline}`, 0], ['Điều kiện tham gia', 'Thành viên, KYC, hạn chế, trạng thái ví và tương thích quy tắc đều là tham chiếu mock chỉ đọc.', 1], ['Quy tắc đã xác nhận', `${ruleVersion} · Ba tuyên bố đã được xác nhận · Snapshot chấp thuận: ${snapshot}`, 2]]
  return <>
    <p className="registration-eyebrow">BƯỚC 4 / 5</p><h2>Kiểm tra và xác nhận đăng ký</h2><p className="registration-support">Xem lại phiên đấu giá, trạng thái điều kiện và các quy tắc bạn đã xác nhận trước khi chuyển sang đặt cọc.</p>
    ...
    {!final && <p id="final-reason" className="registration-validation">Vui lòng xác nhận thông tin trước khi tiếp tục đặt cọc.</p>}
  </>
}

function RegistrationDepositStep({
  auction,
  walletBalance,
  depositedAmount,
  accepted,
  ...
  const alreadyDeposited = depositedAmount > 0
  const missingAmount = Math.max(0, depositAmount - walletBalance)
  const balanceAfterDeposit = alreadyDeposited
    ? walletBalance
    : Math.max(0, walletBalance - depositAmount)

  return <>
    <p className="registration-eyebrow">BƯỚC 5 / 5</p>
    <h2>Đặt cọc và gửi đăng ký</h2>
    <p className="registration-support">Kiểm tra số tiền cọc trước khi xác nhận. Khoản cọc chỉ được trừ khi bạn bấm nút gửi đăng ký bên dưới.</p>

    <section className={`registration-deposit-summary ${alreadyDeposited ? 'is-paid' : ''}`}>
      <div className="registration-deposit-title">
        <Gavel />
        <div>
      ...
      <dl>
        <Metadata label="Giá khởi điểm">{formatMoney(auction.startPrice)}</Metadata>
        <Metadata label="Tiền cọc 10%">{formatMoney(depositAmount)}</Metadata>
        <Metadata label="Số dư ví hiện tại">{formatMoney(walletBalance)}</Metadata>
        <Metadata label={alreadyDeposited ? 'Trạng thái tiền cọc' : 'Số dư sau khi cọc'}>
          {alreadyDeposited ? `Đã đặt cọc ${formatMoney(depositedAmount)}` : formatMoney(balanceAfterDeposit)}
        </Metadata>
      </dl>
    </section>

    {missingAmount > 0 && !alreadyDeposited && (
      <aside className="registration-deposit-warning">
        <AlertCircle />
        <div>
          <strong>Số dư ví còn thiếu {formatMoney(missingAmount)}</strong>
          <p>Bạn cần nạp đủ số dư trước khi có thể đặt cọc và gửi đăng ký.</p>
          <button className="button secondary" type="button" onClick={() => onTopUp(missingAmount)}>
            Nạp bổ sung qua cổng thanh toán demo
          </button>
        </div>
      </aside>
    )}

    <aside className="registration-deposit-policy">
      <ShieldCheck />
      <p>Khoản cọc demo được ghi nhận cho đúng phiên này. Chính sách hoàn hoặc khấu trừ tiền cọc được áp dụng theo {ruleVersion}.</p>
    </aside>

    <label className="registration-ack">
      <input
        type="checkbox"
        checked={accepted}
        onChange={(event) => onAccepted(event.target.checked)}
      />
      Tôi đã kiểm tra số tiền cọc và đồng ý với chính sách đặt cọc của phiên.
    </label>
    {!accepted && <p className="registration-validation">Vui lòng đồng ý với chính sách đặt cọc trước khi gửi đăng ký.</p>}
  </>
}
...
export function AuctionRegistrationWizard() {
  ...
  const depositedAmount = auction ? (auctionDeposits[auction.id] ?? 0) : 0
  const insufficientBalance = depositedAmount === 0 && walletBalance < depositAmount
  ...
  if (done) return <div className="auction-registration-page registration-state"><ShieldCheck/><h1>Đặt cọc và gửi đăng ký thành công</h1><p>Khoản cọc {formatMoney(depositAmount)} đã được ghi nhận cho phiên này. Hồ sơ đang chờ kiểm tra và việc gửi đăng ký chưa đồng nghĩa hồ sơ đã được phê duyệt.</p><div className="registration-submission-status"><CheckCircle2/><span>Đã gửi đăng ký · Đã ghi nhận tiền cọc · Đang chờ xét duyệt</span></div><Link className="button primary" to={`/auctions/${auction.id}/eligibility`}>Xem trạng thái điều kiện</Link></div>

  const submitRegistration = () => {
    if (!final || !depositAccepted || insufficientBalance || sending) return
    setSending(true)
    window.setTimeout(() => {
      if (!depositedAmount) payAuctionDeposit(auction.id, depositAmount)
      setSending(false)
      setDone(true)
    }, 700)
  }

  return <div className="auction-registration-page"><section className="registration-hero"><div className="container"><span>ĐĂNG KÝ THAM GIA</span><h1>Hoàn tất đăng ký phiên đấu giá</h1><p>{auction.assetName}</p></div></section><main className="container registration-wrap"><nav className="registration-stepper" aria-label="Các bước đăng ký">{steps.map(([id, label], index) => <button key={id} className={index === active ? 'active' : ''} aria-current={index === active ? 'step' : undefined} onClick={() => index <= active && go(index)}><b>{index + 1}</b>{label}</button>)}</nav><div className="registration-grid"><section className="registration-card">{active === 0 && <RegistrationSummaryStep auction={auction} acknowledged={summary} onAcknowledge={setSummary}/>} {active === 1 && <RegistrationEligibilityStep scenario={scenario}/>} {active === 2 && <RegistrationRulesStep auction={auction} declarations={rules} onChange={(index: number) => setRules((items) => items.map((item, current) => current === index ? !item : item))} onOpenRules={() => setDrawer(true)}/>} {active === 3 && <RegistrationReviewStep auction={auction} final={final} onFinal={setFinal} go={go}/>} {active === 4 && <RegistrationDepositStep auction={auction} walletBalance={walletBalance} depositedAmount={depositedAmount} accepted={depositAccepted} onAccepted={setDepositAccepted} onTopUp={topUpWallet}/>}<div className="registration-actions">{active > 0 && <button className="button secondary" disabled={sending} onClick={() => go(active - 1)}><ChevronLeft/>Quay lại</button>}{active < 3 && <button className="button primary" disabled={(active === 0 && !summary) || (active === 1 && blocked) || (active === 2 && !rules.every(Boolean))} onClick={() => go(active + 1)}>Tiếp tục</button>}{active === 3 && <button className="button primary" disabled={!final} onClick={() => go(4)}>Tiếp tục đặt cọc</button>}{active === 4 && <button className="button primary" disabled={!final || !depositAccepted || insufficientBalance || sending} onClick={submitRegistration}>{sending ? 'Đang đặt cọc và gửi...' : depositedAmount ? 'Gửi đăng ký' : 'Đặt cọc và gửi đăng ký'}</button>}</div></section><AuctionRegistrationContextCard auction={auction}/></div></main>{drawer && <Overlay title="Quy tắc đấu giá" close={() => setDrawer(false)} closeRef={close}><p>{ruleVersion} áp dụng cho phiên {auction.code} và được sử dụng dưới dạng snapshot chỉ đọc.</p>{[['Giá khởi điểm', formatMoney(auction.startPrice)], ['Bước giá tối thiểu', formatMoney(auction.minimumIncrement)], ['Thời gian', `${startsAt} đến ${endsAt}`], ['Chính sách gia hạn', 'Có bid hợp lệ trong 5 phút cuối sẽ gia hạn 5 phút theo mock rule.'], ['Điều kiện đặt giá hợp lệ', 'Eligibility đang phù hợp, giá đặt không thấp hơn minimum next bid và phiên không paused, closing hoặc closed.'], ['Quy tắc chốt phiên', 'Rank 1 sau khi đóng phiên chỉ là ứng viên ưu tiên, chưa phải người trúng đấu giá chính thức.'], ['Candidate và Financial Mock', 'Financial Mock xác nhận trạng thái thanh toán; Auction UI không tự xác nhận payment hoặc Final Winner.'], ['Hủy phiên và ngoại lệ', 'Phiên có thể cancelled, paused hoặc emergency close theo trạng thái vận hành mock.']].map(([title, text]) => <article key={title}><h3>{title}</h3><p>{text}</p></article>)}</Overlay>}</div>
}
